...
export function MetaSearchDashboard({
  ...
  const [relatedTopicsError, setRelatedTopicsError] = useState<string | null>(
    null
  );
  ...
  // Format price chart data for the chart component
                ...
                <Card className="group/news bg-gray-900 border-gray-700 hover:shadow-[0_0_15px_rgba(255,107,53,0.2)] hover:border-orange-500/40 hover:scale-[1.01] transition-all duration-500 transform-gpu cursor-pointer">
                  ...
                  <CardContent className="space-y-4">
                    {currentNews.map((article) => {
                      const name =
                        article.creator_display_name ||
                        article.creator_name ||
                        "Source";
                      const handle = article.creator_name
                        ? `@${article.creator_name}`
                        : "";
